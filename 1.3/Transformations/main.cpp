...
int main()
{
	...
	*/
	...
	if (reflectCheck(originalSquare, afterSquare, squareLength) == "yes")
	{
		fout << 4 << endl;
		return 0;
	}
    ...
    if (patternChanged(originalSquare, afterSquare, squareLength) == false)
	{
		fout << 6 << endl;
		return 0;
	}
...
int rotationCheck(string originalSquare, string afterSquare, int squareLength, int timesRotated) // checks for 90 and 180 deg and 270 rotation - 5 means not any. 0 means 90 1 is 180 2 270
{
	...
	for (int i = 0; i < sqArea; i++)
	{
		if (rotatedArr[i] != afterSquare[i]) // if not 90deg
		{
			//rotate another 90 deg
            int result = rotationCheck(rotatedArr, afterSquare, squareLength, timesRotated+1);
            //if (result != 5) { return result; } else { return 5; }
